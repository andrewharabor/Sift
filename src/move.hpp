...
namespace Sift {
    enum class MoveType : UInt16 { NORMAL = 0, PROMOTION = 1 << 14, EN_PASSANT = 2 << 14, CASTLING = 3 << 14 };
    ...
    public:
        ...
        constexpr explicit operator std::string() const {
            if (move_ == NULL_MOVE) { return "(none)"; }
            ...
            if (type() == MoveType::PROMOTION) { promotionString = std::string(promotion()); }
        ...
        constexpr PieceType promotion() const noexcept {
            if (type() != MoveType::PROMOTION) { return PieceType::NONE; }
    ...
    public:
        ...
        constexpr USize findIf(Function compare) const noexcept {
            ...
            if (it != moveList_.begin() + size_) { return static_cast<USize>(it - moveList_.begin()); }
            return size_;
        }
