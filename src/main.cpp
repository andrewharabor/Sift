...
#define CLOWNFISH_ENABLE_PERFT_BENCHMARK 0
#endif
...
std::int16_t checkedInt16(int value) {
    assert(value >= -32768 && value <= 32767);
    return static_cast<std::int16_t>(value);
}

std::int16_t deterministicBufferValue(std::size_t index, int seed, int minValue, int maxValue) {
    assert(minValue <= maxValue);
    const std::uint32_t indexValue = static_cast<std::uint32_t>(index + 1U);
    const std::uint32_t seedValue = static_cast<std::uint32_t>(seed + 101);
    const std::uint32_t mixed = (indexValue * 2654435761U) ^ (seedValue * 2246822519U);
    const int range = (maxValue - minValue) + 1;
    const int offset = static_cast<int>(mixed % static_cast<std::uint32_t>(range));
...
int32_t scalarScrelu(std::int16_t input) {
...
void testSIMDBufferAndConstants() {
    ...
    assert((SIMD::ITERATIONS * Constants::SIMD_LANES) == Constants::NNUE_LAYER_SIZE);
...
void testSIMDForward() {
    ...
    for (std::size_t i = 0; i < Constants::NNUE_LAYER_SIZE; ++i) {
        ...
        if (bucket == 0U) {
            boundaryInputs[i] = static_cast<std::int16_t>(-512);
        } else if (bucket == 1U) {
            boundaryInputs[i] = static_cast<std::int16_t>(-1);
        } else if (bucket == 2U) {
            boundaryInputs[i] = static_cast<std::int16_t>(0);
        } else if (bucket == 3U) {
            boundaryInputs[i] = static_cast<std::int16_t>(quantA);
        } else if (bucket == 4U) {
            boundaryInputs[i] = static_cast<std::int16_t>(quantA + 1);
        } else {
            boundaryInputs[i] = static_cast<std::int16_t>(1024);
        }

        const int weightValue = static_cast<int>(i % 9U) - 4;
...
void testSIMDForwardSingleActiveIndex() {
    ...
    for (std::size_t activeIndex = 0; activeIndex < Constants::NNUE_LAYER_SIZE; ++activeIndex) {
        ...
        if (bucket == 0U) {
            inputs[activeIndex] = static_cast<std::int16_t>(-64);
        } else if (bucket == 1U) {
            inputs[activeIndex] = static_cast<std::int16_t>(0);
        } else if (bucket == 2U) {
            inputs[activeIndex] = static_cast<std::int16_t>(67);
        } else if (bucket == 3U) {
            inputs[activeIndex] = static_cast<std::int16_t>(quantA);
        } else {
            inputs[activeIndex] = static_cast<std::int16_t>(quantA + 64);
        }
...
void testSIMDArithmeticPrimitives() {
    for (int caseIndex = 0; caseIndex < 8; ++caseIndex) {
        ...
        fillAlignedBuffer(add1, 300 + (caseIndex * 23), -1000, 1000);
        fillAlignedBuffer(add2, 400 + (caseIndex * 29), -1000, 1000);
        ...
        assertAlignedBuffersEqual(add2, add2Before);
        assertAlignedBuffersEqual(sub1, sub1Before);
        assertAlignedBuffersEqual(sub2, sub2Before);
    }
}
...
void testSIMDComposedOperationRelationships() {
    for (int caseIndex = 0; caseIndex < 6; ++caseIndex) {
        ...
        fillAlignedBuffer(add1, 900 + (caseIndex * 23), -900, 900);
...
PieceFlag pieceFlagForType(PieceType pieceType) {
    if (pieceType == PieceType::PAWN) {
        return PieceFlag::PAWN;
    }
    if (pieceType == PieceType::KNIGHT) {
        return PieceFlag::KNIGHT;
    }
    if (pieceType == PieceType::BISHOP) {
        return PieceFlag::BISHOP;
    }
    if (pieceType == PieceType::ROOK) {
        return PieceFlag::ROOK;
    }
    if (pieceType == PieceType::QUEEN) {
        return PieceFlag::QUEEN;
    }
    if (pieceType == PieceType::KING) {
        return PieceFlag::KING;
    }
    assert(false);
    return static_cast<PieceFlag>(0);
}
...
void assertMovesFromAllowedPieces(const Position &position, const MoveList &moveList, PieceFlag allowedPieceFlags) {
    for (const Move move : moveList) {
        const Piece piece = position.pieceAt(move.from());
        assert(piece != Piece::NONE);
        assert(allowedPieceFlags & pieceFlagForType(piece.type()));
    }
}
...
void assertMoveGenerationPartition(const Position &position, PieceFlag pieces = PieceFlag::ALL) {
    MoveList allMoves;
...
void testMoveGeneratorPieceFlagsFiltering() {
    ...
    MoveGenerator::legal(position, knightMoves, PieceFlag::KNIGHT);
    assert(!knightMoves.empty());
    assertMovesFromAllowedPieces(position, knightMoves, PieceFlag::KNIGHT);
    for (const Move move : knightMoves) {
        assert(move.from() == Square::SQUARE_F3);
    }

    MoveList pawnMoves;
    MoveGenerator::legal(position, pawnMoves, PieceFlag::PAWN);
    assert(!pawnMoves.empty());
    assertMovesFromAllowedPieces(position, pawnMoves, PieceFlag::PAWN);
    for (const Move move : pawnMoves) {
        assert(move.from() == Square::SQUARE_E4);
    }

    const PieceFlag kingAndPawn = PieceFlag::KING | PieceFlag::PAWN;
    ...
    MoveList noMoves;
    MoveGenerator::legal(position, noMoves, static_cast<PieceFlag>(0));
...
void testMoveGeneratorPromotionsAndMoveTypes() {
    ...
    assertMoveGenerationPartition(position, PieceFlag::PAWN);
...
void testMoveGeneratorCastlingRules() {
    ...
    MoveGenerator::legal(bothSides, bothSidesMoves, PieceFlag::KING);
    ...
    MoveGenerator::legal(blockedPath, blockedPathMoves, PieceFlag::KING);
    ...
    MoveGenerator::legal(attackedTransit, attackedTransitMoves, PieceFlag::KING);
    ...
    MoveGenerator::legal(kingInCheck, kingInCheckMoves, PieceFlag::KING);
    ...
    MoveGenerator::legal(blackBothSides, blackBothSidesMoves, PieceFlag::KING);
...
void testMoveGeneratorCheckEvasionsAndDoubleCheck() {
    ...
    MoveGenerator::legal(singleCheck, queenResponses, PieceFlag::QUEEN);
...
void testMoveGeneratorPinsAndEnPassantLegality() {
    ...
    MoveGenerator::legal(pinnedKnight, pinnedKnightMoves, PieceFlag::KNIGHT);
...
void testMoveGeneratorMultiChecksByKnightsAndPawns() {
    ...
    constexpr PieceFlag nonKingFlags = PieceFlag::PAWN | PieceFlag::KNIGHT | PieceFlag::BISHOP | PieceFlag::ROOK | PieceFlag::QUEEN;
...
int main() {
    ...
    testMoveList();
    testMoveListExtras();
    testSIMDBufferAndConstants();
    testSIMDForward();
    testSIMDForwardSingleActiveIndex();
    testSIMDArithmeticPrimitives();
    testSIMDComposedOperationRelationships();
    testAttacksAndMagic();
    testAttacksBetween();
