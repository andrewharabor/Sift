...
namespace Sift {
    ...
    public:
        ...
        static FORCE_INLINE std::pair<Vector, Vector> permuteMailbox(const Permutation& perm, Vector maskedMailbox) noexcept {
            ...
            const Vector permuted{{halfSwizzler(maskedMailbox.raw[0], maskedMailbox.raw[1], perm.indices.raw[0]),
                halfSwizzler(maskedMailbox.raw[0], maskedMailbox.raw[1], perm.indices.raw[1])}};
            const Vector bits{{_mm256_andnot_si256(perm.invalid.raw[0], _mm256_shuffle_epi8(pieceBits, permuted.raw[0])),
                _mm256_andnot_si256(perm.invalid.raw[1], _mm256_shuffle_epi8(pieceBits, permuted.raw[1]))}};
        ...
        static FORCE_INLINE std::pair<Vector, Vector> permuteMailbox(const Permutation& perm, const std::span<const Piece, 64> mailbox,
            ...
            const Vector maskedMailbox{{_mm256_blendv_epi8(mailboxVec.raw[0], noneVec, _mm256_cmpeq_epi8(iota.raw[0], ignoreVec)),
                _mm256_blendv_epi8(mailboxVec.raw[1], noneVec, _mm256_cmpeq_epi8(iota.raw[1], ignoreVec))}};
        ...
        static FORCE_INLINE std::pair<Vector, Vector> permuteMailbox(const Permutation& perm, Vector mailbox) {
            ...
            const Vector bits = Vector{vandq_u8(vqtbl1q_u8(pieceBits, permuted[0]), perm.valid[0]),
                vandq_u8(vqtbl1q_u8(pieceBits, permuted[1]), perm.valid[1]), vandq_u8(vqtbl1q_u8(pieceBits, permuted[2]), perm.valid[2]),
                vandq_u8(vqtbl1q_u8(pieceBits, permuted[3]), perm.valid[3])};
            return {permuted, bits};
        }
        ...
        static FORCE_INLINE std::pair<Vector, Vector> permuteMailbox(const Permutation& perm, const std::span<const Piece, 64> mailbox,
            ...
            const auto mailboxVec = Vector::load(mailbox.data());
            const Vector maskedMailbox{vbslq_u8(vceqq_u8(iota[0], ignoreVec), noneVec, mailboxVec[0]),
                vbslq_u8(vceqq_u8(iota[1], ignoreVec), noneVec, mailboxVec[1]),
                vbslq_u8(vceqq_u8(iota[2], ignoreVec), noneVec, mailboxVec[2]),
                vbslq_u8(vceqq_u8(iota[3], ignoreVec), noneVec, mailboxVec[3])};
            return permuteMailbox(perm, maskedMailbox);
        }
