...
struct Vec {
    ...
    inline Vec flipped() const noexcept { return Vec{_mm512_shuffle_i64x2(raw, raw, 0b01001110)}; }
};
...
struct Vec {
    ...
    inline Vec flipped() const noexcept { return Vec{{raw[1], raw[0]}}; }
    ...
    static inline Vec load(const void *ptr) noexcept { return Vec{{_mm256_loadu_si256(static_cast<const __m256i *>(ptr)), _mm256_loadu_si256(static_cast<const __m256i *>(ptr) + 1)}}; }

    template<typename Type>
    static inline Vec cast(const Type &value) noexcept requires(sizeof(Type) == sizeof(__m256i) * 2) { return load(&value); }
};
...
struct Vec {
    ...
    explicit inline Vec(uint8x16x4_t raw) noexcept : raw(raw) {}
    explicit inline Vec(uint8x16_t value0, uint8x16_t value1, uint8x16_t value2, uint8x16_t value3) noexcept : raw({value0, value1, value2, value3}) {}

    inline Vec flipped() const noexcept { return Vec{raw.val[2], raw.val[3], raw.val[0], raw.val[1]}; }
    ...
    static inline Vec load(const void *ptr) { return Vec(vld1q_u8_x4(static_cast<const UInt8 *>(ptr))); }

    template<typename Type>
    static inline Vec cast(const Type &value) requires(sizeof(Type) == sizeof(uint8x16x4_t)) { return load(&value); }
...
public:
    ...
    static inline BitRays incomingSliders(Vec bits, BitRays closest) noexcept {
    ...
    static inline BitRays closestOccupied(Vec bits) noexcept {
        const Vec unoccupied{{_mm256_cmpeq_epi8(bits.raw[0], _mm256_setzero_si256()), _mm256_cmpeq_epi8(bits.raw[1], _mm256_setzero_si256())}};
    ...
    static inline BitRays incomingAttackers(Vec bits, BitRays closest) noexcept {
        ...
        const Vec vec{{_mm256_cmpeq_epi8(_mm256_and_si256(bits.raw[0], mask.raw[0]), _mm256_setzero_si256()), _mm256_cmpeq_epi8(_mm256_and_si256(bits.raw[1], mask.raw[1]), _mm256_setzero_si256())}};
        return ~vec.mask() & closest;
    }

    static inline BitRays incomingSliders(Vec bits, BitRays closest) noexcept {
        const auto mask = Vec::cast(INCOMING_SLIDER_MASK);
        const Vec vec{{_mm256_cmpeq_epi8(_mm256_and_si256(bits.raw[0], mask.raw[0]), _mm256_setzero_si256()), _mm256_cmpeq_epi8(_mm256_and_si256(bits.raw[1], mask.raw[1]), _mm256_setzero_si256())}};
    ...
    static inline BitRays closestOccupied(Vec bits) {
        const Vec occupiedVec{vtstq_u8(bits[0], bits[0]), vtstq_u8(bits[1], bits[1]), vtstq_u8(bits[2], bits[2]), vtstq_u8(bits[3], bits[3])};
    ...
    static inline BitRays incomingAttackers(Vec bits, BitRays closest) {
        ...
        const Vec vec{vtstq_u8(bits[0], mask[0]), vtstq_u8(bits[1], mask[1]), vtstq_u8(bits[2], mask[2]), vtstq_u8(bits[3], mask[3])};
        return vec.mask() & closest;
    }

    static inline BitRays incomingSliders(Vec bits, BitRays closest) {
        const auto mask = Vec::load(INCOMING_SLIDER_MASK.data());
        const Vec vec{vtstq_u8(bits[0], mask[0]), vtstq_u8(bits[1], mask[1]), vtstq_u8(bits[2], mask[2]), vtstq_u8(bits[3], mask[3])};
        return vec.mask() & closest & 0xFEFEFEFEFEFEFEFE;
    }
