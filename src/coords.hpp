...
namespace Sift {
    class File {
    public:
        enum class FileEnum : UInt8 { A, B, C, D, E, F, G, H, NONE };
    ...
    class Rank {
    public:
        enum class RankEnum : UInt8 { FIRST, SECOND, THIRD, FOURTH, FIFTH, SIXTH, SEVENTH, EIGHTH, NONE };
    ...
    public:
        ...
        constexpr Direction(Int8 direction) noexcept : direction_(static_cast<DirectionEnum>(direction)) {
            assert(direction == 8 || direction == 1 || direction == -8 || direction == -1 || direction == 9 || direction == -7 ||
                   direction == -9 || direction == 7 || direction == 0);
        }

        constexpr Direction(DirectionEnum direction, Color color) noexcept : direction_(direction) {
            assert(color != Color::NONE);
            if (color == Color::BLACK) { direction_ = static_cast<DirectionEnum>(-static_cast<Int8>(direction_)); }
        }
    ...
    public:
        ...
        constexpr explicit operator std::string() const noexcept {
            if (square_ == SquareEnum::NONE) { return ""; }
        ...
        constexpr Square operator-(Direction direction) const noexcept { return *this + (-direction); }
