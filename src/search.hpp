...
namespace Sift {
    ...
    enum class ThreadFlag : UInt8 { IDLE, START, STOP };
    ...
    struct SearchThread {
        ...
        void initMoves() {
            ...
            for (const Move move : moves) { rootMoves.push_back(RootMove(move)); }
        }

        void sortSearchedMoves() {
            auto compare = [](const RootMove& move1, const RootMove& move2) {
                if (move1.score == move2.score) { return move1.prevScore > move2.prevScore; }
        ...
        USize rootMoveIdx(Move move) {
            ...
            if (it != rootMoves.end()) { return static_cast<USize>(it - rootMoves.begin()); }
    ...
    public:
        ...
        Search(SearchInfoCallback uciSearchInfo, BestMoveCallback uciBestMove, CurrMoveCallback uciCurrMove) :
            tt_(1),
            contempt_({0, 0}),
            timeManager_(),
            uciSearchInfo_(std::move(uciSearchInfo)),
            uciBestMove_(std::move(uciBestMove)),
            uciCurrMove_(std::move(uciCurrMove)),
            printInfo_(true) {
            setTimeUp(false);
            setThreads();
            resizeTT();
        ...
        void newGame() noexcept {
            ...
            for (USize node = 0; node < NUMA::nodeCount(); node++) { sharedHistory_.getForNode(node)->reset(); }
        }
        ...
        void run(const Position& position, const SearchLimits& limits) noexcept {
            ...
            if (legalMoves.empty() || position.draw(0, legalMoves.empty())) {
                if (printInfo_) { uciBestMove_(Move::NULL_MOVE); }
                return;
            }
        ...
        bool running() const noexcept {
            for (const auto& thread : threads_) {
                ...
                if (thread->flag == ThreadFlag::START) { return true; }
            }
            return false;
        }
    ...
    private:
        ...
        void searchRoot(SearchThread& thread) noexcept {
            ...
            for (Int32 depth = 1; depth <= maxDepth; depth++) {
                ...
                for (RootMove& rootMove : thread.rootMoves) { rootMove.prevScore = rootMove.score; }
                ...
                for (thread.pvIdx = 0; thread.pvIdx < std::min(multiPV_, thread.rootMoves.size()); thread.pvIdx++) {
                    ...
                    if (depth >= WINDOW_MIN_DEPTH) {
                        ...
                        delta += static_cast<Int32>(static_cast<Int64>(std::abs(rootMove.averageSquaredScore)) *
                                                    static_cast<Int64>(WINDOW_SQUARED_SCORE_SCALE) / 1048576);
                    }

                    if (depth >= OPTIMISM_MIN_DEPTH) {
                        const Int32 optimism =
                            OPTIMISM_SCORE_SCALE * rootMove.averageScore / (std::abs(rootMove.averageScore) + OPTIMISM_DIVISOR_OFFSET);
                ...
                if (printInfo_ && thread.main()) {
                    for (USize idx = 0; idx < std::min(multiPV_, thread.rootMoves.size()); idx++) { printSearchInfo(thread, idx, depth); }
                }

                if (thread.main() && timeManager_.stopSoft(thread.limits, depth, thread.bestMove().move, thread.bestMove().score,
                                         thread.bestMove().nodes, thread.loadNodes())) {
                    setTimeUp(true);
                }
            ...
            if (thread.main()) {
                if (thread.limits.infinite) {
                    while (!timeUp()) { std::this_thread::yield(); }
                }

                setTimeUp(true);

                for (auto& otherThread : threads_) {
                    if (!otherThread->main()) { otherThread->wait(); }
                }

                if (printInfo_) {
                    const SearchThread& bestThread = selectThread();

                    if (!bestThread.main()) { printSearchInfo(bestThread, 0, bestThread.depth); }
        ...
        Int32 search(SearchThread& thread, Int32 fdepth, Int32 alpha, Int32 beta, USize ply, USize moveStackIdx, bool cutNode) noexcept {
            ...
            if constexpr (PV_NODE) { thread.selDepth = std::max(thread.selDepth, static_cast<Int32>(ply + 1)); }
            ...
            if (positionDraw(thread, ply)) { return drawScore; }

            if (ply >= MAX_PLY) {
                return (inCheck) ? 0 :
                                   Eval::adjusted(position, thread.nnue, thread.optimism, contempt_,
                                       sharedHistory->correction(position, histStack, ply));
            }

            if (fdepth <= 0) { return qsearch<PV_NODE>(thread, alpha, beta, ply, moveStackIdx); }
            ...
            if (!excluded) {
                ...
                if constexpr (!PV_NODE) {
                    if (ttHit && ttEntry.fdepth >= fdepth && (ttEntry.score <= alpha || cutNode) &&
                        ((ttEntry.bound == TTBound::EXACT) || (ttEntry.bound == TTBound::LOWER && ttEntry.score >= beta) ||
                            (ttEntry.bound == TTBound::UPPER && ttEntry.score <= alpha))) {
                        if (ttEntry.score >= beta && ttEntry.move != Move::NULL_MOVE && position.quiet(ttEntry.move) &&
                            position.legal(ttEntry.move)) {
                            const Int32 bonus =
                                History::bonus(fdepth, TT_CUTOFF_BONUS_DEPTH_SCALE, TT_CUTOFF_BONUS_OFFSET, TT_CUTOFF_BONUS_MAX);
                            history.updateQuietHists(position, histStack, ttEntry.move, ply, bonus);
                        }

                        if (static_cast<Int32>(position.halfmoveClock()) < TT_CUTOFF_MAX_HALFMOVES) { return ttEntry.score; }
                    }
                }
            }
            ...
            if (fdepth >= IIR_MIN_FDEPTH && !excluded && (PV_NODE || cutNode) &&
                (!ttHit || (ttEntry.move != Move::NULL_MOVE && ttEntry.fdepth + IIR_TT_FDEPTH_OFFSET < fdepth))) {
            ...
            if (!excluded) {
                if (inCheck) {
                    ...
                    rawStaticEval =
                        (ttHit && ttEntry.staticEval != Score::NONE) ? ttEntry.staticEval : Eval::raw(position, thread.nnue, contempt_);
                    ...
                    if (ttHit &&
                        ((ttEntry.bound == TTBound::EXACT) || (ttEntry.bound == TTBound::LOWER && ttEntry.score >= curr.staticEval) ||
                            (ttEntry.bound == TTBound::UPPER && ttEntry.score <= curr.staticEval))) {
                        curr.eval = ttEntry.score;
                    }

                    if (!ttHit) { tt_.write(position.hash(), 0, Score::NONE, rawStaticEval, Move::NULL_MOVE, 0, ttPV, TTBound::NONE); }
                }
                ...
                if constexpr (!ROOT_NODE) {
                    if (!inCheck && prev.move != Move::NULL_MOVE && prev.quiet && prev.staticEval != Score::NONE) {
                        ...
                        const Int32 bonus = std::clamp(EVAL_POLICY_BONUS_BASE + gain * EVAL_POLICY_BONUS_GAIN_SCALE, EVAL_POLICY_BONUS_MIN,
                            EVAL_POLICY_BONUS_MAX);
            ...
            if constexpr (!PV_NODE) {
                if (!inCheck && !excluded) {
                    if (prev.freduction >= HINDSIGHT_FEXT_MIN_FREDUCTION && prev.staticEval != Score::NONE &&
                        curr.staticEval + prev.staticEval <= 0) {
                        fdepth += HINDSIGHT_FEXTENSION;
                    }

                    if (fdepth >= HINDSIGHT_FRED_MIN_FDEPTH && prev.freduction >= HINDSIGHT_FRED_MIN_FREDUCTION &&
                        prev.staticEval != Score::NONE && curr.staticEval + prev.staticEval >= HINDSIGHT_FRED_MARGIN) {
                    ...
                    if (fdepth <= RFP_MAX_FDEPTH && curr.eval - rfpMargin >= beta) {
                        return (!Score::decisive(curr.eval) && !Score::decisive(beta)) ?
                                   Utils::linInterp<1024>(curr.eval, beta, RFP_FAIL_FIRM_T) :
                                   curr.eval;
                    }
                    ...
                    if (fdepth <= RAZORING_MAX_FDEPTH && curr.eval + razoringMargin <= alpha) {
                        if (fdepth <= RAZORING_FULL_ALWAYS_MAX_FDEPTH ||
                            (fdepth <= RAZORING_FULL_MAX_FDEPTH && curr.eval + razoringMargin + RAZORING_FULL_MARGIN <= alpha)) {
                        ...
                        if (score <= razoringAlpha) { return score; }
                    ...
                    if (fdepth >= NMP_MIN_FDEPTH && ply >= thread.nmpMinPly && curr.staticEval >= beta + nmpBetaMargin &&
                        position.nullPly() > 0 && !(ttEntry.bound == TTBound::UPPER && ttEntry.score < beta) &&
                        position.nonPawnMaterial(position.sideToMove())) {
                        ...
                        const Int32 freduction = [&] {
                            ...
                            freduction +=
                                std::min((curr.staticEval - beta) * NMP_FRED_STATIC_EVAL_DIFF_SCALE, NMP_FRED_STATIC_EVAL_DIFF_MAX);
                            return freduction;
                        }();
                        ...
                        if (score >= beta) {
                            ...
                            if (verifScore >= beta) { return verifScore; }
                        }
                    }
                    ...
                    if (fdepth >= PROBCUT_MIN_FDEPTH && !ttPV && !Score::decisive(beta) && (ttMove == Move::NULL_MOVE || noisyTTMove) &&
                        (!ttHit || ttEntry.fdepth < probcutFdepth || ttEntry.score >= probcutBeta)) {
                        ...
                        while ((move = moveOrder.next()) != Move::NULL_MOVE) {
                            if (!position.see(move, seeMargin)) { continue; }
                            ...
                            if (score >= probcutBeta) {
                                score = -search<false, false>(thread, probcutFdepth - FDEPTH_SCALE, -probcutBeta, -probcutBeta + 1, ply + 1,
                                    moveStackIdx + 1, !cutNode);
                            }
                            ...
                            if (score >= probcutBeta) {
                                tt_.write(position.hash(), static_cast<Int32>(ply), score, rawStaticEval, move, probcutFdepth, false,
                                    TTBound::LOWER);
                                return score;
                            }
                        }
            ...
            if (!excluded) {
                ...
                if (!ttPV && ttEntry.bound == TTBound::LOWER && ttEntry.fdepth + MINI_PROBCUT_TT_FDEPTH_OFFSET >= fdepth &&
                    ttEntry.score >= miniProbcutBeta && !Score::decisive(ttEntry.score) && !Score::decisive(beta)) {
                    return miniProbcutBeta;
                }

                if (fdepth >= IIR2_MIN_FDEPTH && cutNode && !ttHit) { fdepth -= IIR2_FREDUCTION; }
            }
            ...
            while ((move = moveOrder.next()) != Move::NULL_MOVE) {
                if constexpr (ROOT_NODE) {
                    if (thread.rootMoveIdx(move) >= thread.rootMoves.size()) { continue; }
                ...
                if (move == curr.excludedMove) { continue; }

                const bool quiet = position.quiet(move);

                const Int32 historyScore =
                    (quiet) ? history.quietScore(position, histStack, move, ply) : history.noisyScore(position, move);
                ...
                if constexpr (!ROOT_NODE) {
                    if (!Score::loss(bestScore)) {
                        ...
                        if (quiet) {
                            const Int32 lmpMargin = LMP_TABLE[improving][static_cast<USize>(fdepth / FDEPTH_SCALE)] +
                                                    historyScore * LMP_MARGIN_HISTORY_SCALE / 8388608;
                            if (movesTried >= lmpMargin) {
                                moveOrder.skipQuiets();
                                continue;
                            }

                            const Int32 quietHistoryPruningMargin =
                                QUIET_HISTORY_PRUNING_MARGIN_DEPTH_SCALE * fdepth / FDEPTH_SCALE + QUIET_HISTORY_PRUNING_MARGIN_OFFSET;
                            if (lmrFdepth <= QUIET_HISTORY_PRUNING_MAX_FDEPTH && historyScore < quietHistoryPruningMargin) {
                                moveOrder.skipQuiets();
                                continue;
                            }

                            const Int32 quietFPMargin = QUIET_FP_MARGIN_BASE + QUIET_FP_MARGIN_DEPTH_SCALE * fdepth / FDEPTH_SCALE +
                                                        historyScore / QUIET_FP_MARGIN_HISTORY_DIVISOR;
                            if (lmrFdepth <= QUIET_FP_MAX_FDEPTH && !inCheck && !position.directCheck(move) &&
                                curr.staticEval + quietFPMargin <= alpha) {
                                moveOrder.skipQuiets();
                                continue;
                            }
                        } else {
                            const Int32 noisyHistoryPruningMargin =
                                NOISY_HISTORY_PRUNING_MARGIN_DEPTH_SCALE * fdepth / FDEPTH_SCALE * fdepth / FDEPTH_SCALE +
                                NOISY_HISTORY_PRUNING_MARGIN_OFFSET;
                            if (lmrFdepth <= NOISY_HISTORY_PRUNING_MAX_FDEPTH && historyScore < noisyHistoryPruningMargin) { continue; }

                            const Int32 bnfpMargin = BNFP_MARGIN_BASE + BNFP_MARGIN_DEPTH_SCALE * fdepth / FDEPTH_SCALE +
                                                     historyScore / BNFP_MARGIN_HISTORY_DIVISOR;
                            if (moveOrder.stage() >= MoveOrderStage::BAD_NOISY && lmrFdepth <= BNFP_MAX_FDEPTH && !inCheck &&
                                curr.staticEval + bnfpMargin <= alpha) {
                                break;
                            }

                            const Int32 captureFPMargin = CAPTURE_FP_MARGIN_BASE + CAPTURE_FP_MARGIN_DEPTH_SCALE * fdepth / FDEPTH_SCALE;
                            if (position.capture(move) && lmrFdepth <= CAPTURE_FP_MAX_FDEPTH && !inCheck &&
                                curr.staticEval + captureFPMargin <= alpha) {
                                continue;
                            }
                        }

                        const Int32 seePruningMargin =
                            (quiet) ? (SEE_PRUNING_MARGIN_QUIET_DEPTH_SCALE * fdepth / FDEPTH_SCALE * fdepth / FDEPTH_SCALE) :
                                      (std::min(SEE_PRUNING_MARGIN_NOISY_DEPTH_SCALE * fdepth / FDEPTH_SCALE -
                                                    historyScore / SEE_PRUNING_MARGIN_NOISY_HISTORY_DIVISOR,
                                          0));
                        if (moveOrder.stage() >= MoveOrderStage::QUIET && !position.see(move, seePruningMargin)) { continue; }
                    }
                }
                ...
                if constexpr (!ROOT_NODE) {
                    if (move == ttMove && !excluded) {
                        if (fdepth >= SE_MIN_FDEPTH_BASE + ttPV * SE_MIN_FDEPTH_TT_PV_SCALE &&
                            ttEntry.fdepth + SE_TT_FDEPTH_OFFSET >= fdepth && ttEntry.bound != TTBound::UPPER &&
                            !Score::decisive(ttEntry.score)) {
                            const Int32 seBetaMargin = SE_BETA_MARGIN_BASE + SE_BETA_MARGIN_PREV_PV_SCALE * (ttPV && !PV_NODE);
                            const Int32 seBeta =
                                std::max(ttEntry.score - fdepth * seBetaMargin / (FDEPTH_SCALE * 128), Score::MATED_IN_MAX);
                            ...
                            if constexpr (!PV_NODE) {
                                if (fdepth <= MLDE_MAX_FDEPTH && score < seBeta + MLDE_MARGIN) { fextension += MLDE_FEXTENSION; }
                            }
                        } else if (fdepth <= LDSE_MAX_FDEPTH && !inCheck && ttEntry.bound == TTBound::LOWER) {
                            fextension += (curr.staticEval <= alpha - LDSE_SINGLE_FEXT_MARGIN) * LDSE_SINGLE_FEXTENSION +
                                          (!PV_NODE && !noisyTTMove && ttEntry.fdepth + LDSE_DOUBLE_FEXT_TT_FDEPTH_OFFSET >= fdepth &&
                                              curr.staticEval <= alpha - LDSE_DOUBLE_FEXT_MARGIN) *
                                              LDSE_DOUBLE_FEXTENSION;
                        }
                    }
                }

                if (fdepth >= CUT_NODE_FRED_MIN_FDEPTH && cutNode && move != ttMove) { fdepth -= CUT_NODE_FREDUCTION; }
                ...
                if (fdepth >= LMR_MIN_FDEPTH && movesTried >= LMR_MIN_MOVES + ROOT_NODE) {
                    ...
                    if (score > alpha) {
                        const bool doDeeper =
                            score > bestScore + DEEPER_SEARCH_MARGIN_BASE + DEEPER_SEARCH_MARGIN_DEPTH_SCALE * newFdepth / FDEPTH_SCALE;
                        newFdepth += doDeeper * DEEPER_SEARCH_FEXTENSION;

                        const bool doShallower = score < bestScore + SHALLOWER_SEARCH_MARGIN_BASE +
                                                             SHALLOWER_SEARCH_MARGIN_DEPTH_SCALE * newFdepth / FDEPTH_SCALE;
                        ...
                        if (quiet && (score <= alpha || score >= beta) && !timeUp()) {
                            Int32 bonus =
                                (score >= beta) ?
                                    History::bonus(newFdepth, POST_LMR_BONUS_DEPTH_SCALE, POST_LMR_BONUS_OFFSET, POST_LMR_BONUS_MAX) :
                                    -History::bonus(newFdepth, POST_LMR_PENALTY_DEPTH_SCALE, POST_LMR_PENALTY_OFFSET, POST_LMR_PENALTY_MAX);
                ...
                if constexpr (ROOT_NODE) {
                    ...
                    if (movesTried == 1 || score > alpha) {
                        ...
                        for (Move pvMove : next.pv) { rootMove.pv.add(pvMove); }
                    } else {
                ...
                if (score > alpha) {
                    ...
                    if constexpr (PV_NODE) {
                        curr.pv.clear();
                        curr.pv.add(move);
                        for (Move pvMove : next.pv) { curr.pv.add(pvMove); }
                    }
                }
            ...
            if (bestMove != Move::NULL_MOVE) {
                ...
                if (position.quiet(bestMove)) {
                    ...
                    const Int32 quietPenalty =
                        -History::bonus(historyFdepth, QUIET_PENALTY_DEPTH_SCALE, QUIET_PENALTY_OFFSET, QUIET_PENALTY_MAX);
                    for (const Move quietMove : moves.failLowQuiets) {
                        history.updateQuietHists(position, histStack, quietMove, ply, quietPenalty);
                    }

                    const Int32 noisyPenalty = -History::bonus(historyFdepth, QUIET_MOVE_NOISY_PENALTY_DEPTH_SCALE,
                        QUIET_MOVE_NOISY_PENALTY_OFFSET, QUIET_MOVE_NOISY_PENALTY_MAX);
                    for (const Move noisyMove : moves.failLowNoisies) { history.updateNoisyHists(position, noisyMove, noisyPenalty); }
                } else {
                    ...
                    const Int32 noisyPenalty = -History::bonus(historyFdepth, NOISY_MOVE_NOISY_PENALTY_DEPTH_SCALE,
                        NOISY_MOVE_NOISY_PENALTY_OFFSET, NOISY_MOVE_NOISY_PENALTY_MAX);
                    for (const Move noisyMove : moves.failLowNoisies) { history.updateNoisyHists(position, noisyMove, noisyPenalty); }
                }
                ...
                if (prev.quiet) {
                    ...
                    const Int32 weight = [&] {
                        ...
                        weight += PCM_WEIGHT_PREV_STATIC_EVAL_SCALE *
                                  (prev.staticEval != Score::NONE && bestScore < -prev.staticEval - PCM_WEIGHT_PREV_MARGIN);
            ...
            if (!excluded) {
                if (!inCheck && (bestMove == Move::NULL_MOVE || position.quiet(bestMove)) &&
                    (bound == TTBound::EXACT || (bound == TTBound::LOWER && bestScore > curr.staticEval) ||
                        (bound == TTBound::UPPER && bestScore < curr.staticEval))) {
        ...
        Int32 qsearch(SearchThread& thread, Int32 alpha, Int32 beta, USize ply, USize moveStackIdx) noexcept {
            ...
            if constexpr (!PV_NODE) {
                if (ttHit && ((ttEntry.bound == TTBound::EXACT) || (ttEntry.bound == TTBound::LOWER && ttEntry.score >= beta) ||
                                 (ttEntry.bound == TTBound::UPPER && ttEntry.score <= alpha))) {
            ...
            if (inCheck) {
                ...
                rawStaticEval =
                    (ttHit && ttEntry.staticEval != Score::NONE) ? ttEntry.staticEval : Eval::raw(position, thread.nnue, contempt_);
                curr.staticEval =
                    Eval::adjust(rawStaticEval, position, thread.optimism, sharedHistory->correction(position, histStack, ply));
                curr.eval = curr.staticEval;
                if (ttHit && ((ttEntry.bound == TTBound::EXACT) || (ttEntry.bound == TTBound::LOWER && ttEntry.score >= curr.staticEval) ||
                                 (ttEntry.bound == TTBound::UPPER && ttEntry.score <= curr.staticEval))) {
                    curr.eval = ttEntry.score;
                }
                ...
                if (curr.eval >= beta) {
                    return (!Score::decisive(curr.eval) && !Score::decisive(beta)) ?
                               Utils::linInterp<1024>(curr.eval, beta, QSEARCH_FAIL_FIRM_T) :
                               curr.eval;
                }

                if (curr.eval > alpha) { alpha = curr.eval; }
            }
            ...
            while ((move = moveOrder.next()) != Move::NULL_MOVE) {
                if (!Score::loss(bestScore)) {
                    if (movesTried >= QSEARCH_MAX_MOVES) { break; }
                    ...
                    if (!position.see(move, QSEARCH_SEE_PRUNING_MARGIN)) { continue; }
                }
                ...
                if (!Score::loss(score)) { moveOrder.skipQuiets(); }
            ...
            if (inCheck && movesTried == 0) { return Score::matedIn(static_cast<Int32>(ply)); }
        ...
        const SearchThread& selectThread() const noexcept {
            if (threads_.size() == 1) { return *threads_[0]; }

            Int32 lowestRootScore = Score::MAX;
            for (const auto& thread : threads_) {
                const Int32 score = thread->bestMove().score;
                if (score != Score::NONE && score < lowestRootScore) { lowestRootScore = score; }
            }

            const auto threadWeight = [&](const SearchThread& thread) {
                return (thread.bestMove().score - lowestRootScore + THREAD_WEIGHT_SCORE_OFFSET) * thread.depth;
            };
            ...
            for (const auto& thread : threads_) {
                if (thread->bestMove().score != Score::NONE) { bestMoveVotes(*thread) += threadWeight(*thread); }
            }
            ...
            for (USize i = 1; i < threads_.size(); i++) {
                ...
                if (Score::win(bestScore)) {
                    if (score > bestScore) { updateBestThread(thread); }
                    continue;
                }

                if (Score::loss(bestScore)) {
                    if (Score::loss(score) && score < bestScore) { updateBestThread(thread); }
                    continue;
                }
                ...
                if (votes == bestVotes && (threadWeight(*thread) * (thread->bestMove().pv.size() > 2) >
                                              threadWeight(*bestThread) * (bestThread->bestMove().pv.size() > 2))) {
                    updateBestThread(thread);
                    continue;
                }
            }
        ...
        void printSearchInfo(const SearchThread& thread, USize pvIdx, Int32 depth) const noexcept {
            ...
            info.nodes = 0;
            for (auto& searchThread : threads_) { info.nodes += searchThread->loadNodes(); }
