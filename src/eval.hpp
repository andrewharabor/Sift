...
public:
    static constexpr Int32 evaluate(const Position &position) noexcept {
        ...
        for (UInt8 i = 0; i < 64; i++) {
            ...
            if (piece != Piece::NONE) {
                const UInt8 color = static_cast<UInt8>(piece.color());
                const UInt8 pieceType = static_cast<UInt8>(piece.type());
                UInt8 squareIndex = i;
                ...
                for (UInt8 phase = 0; phase < 2; phase++) {
                    scores[phase] += (PIECE_VALUES[phase][pieceType] + PIECE_TABLES[phase][pieceType][squareIndex]) * colorFactors[color];
                }
                gamePhase += PIECE_PHASES[pieceType];
            }
        }

        gamePhase = std::min(gamePhase, TOTAL_PHASE);
        return (scores[0] * gamePhase + scores[1] * (TOTAL_PHASE - gamePhase)) / TOTAL_PHASE;
    }
