...
namespace Sift {
    ...
    public:
        ...
        constexpr void update(Int32 bonus) noexcept { update(bonus, value_); }

        constexpr void update(Int32 bonus, Int32 base) noexcept {
            value_ = static_cast<Int16>(std::clamp(static_cast<Int32>(value_) + bonus - base * std::abs(bonus) / MAX, MIN, MAX));
        }
    ...
    using WinningThreatsCorrHistTable = SharedCorrHistTable;
    ...
    public:
        ...
        Int32 quietScore(const Position& position, std::span<const HistoryStackEntry> stack, Move move, USize ply) const noexcept {
            ...
            score += CONT1_HIST_WEIGHT * contHistScore(stack, ply, 1, move, movedPiece);
            score += CONT2_HIST_WEIGHT * contHistScore(stack, ply, 2, move, movedPiece);
            score += CONT4_HIST_WEIGHT * contHistScore(stack, ply, 4, move, movedPiece);
            score += CONT6_HIST_WEIGHT * contHistScore(stack, ply, 6, move, movedPiece);
            return score / 1024;
        }

        Int32 noisyScore(const Position& position, Move move) const noexcept {
            assert(move != Move::NULL_MOVE);
            return CAPTURE_HIST_WEIGHT * captHistScore(move, position.threats(), position.movedPiece(move), position.capturedPiece(move)) /
                   1024;
        }
        ...
        void updateContHist(const Position& position, std::span<const HistoryStackEntry> stack, Move move, USize ply,
            Int32 bonus) noexcept {
            assert(move != Move::NULL_MOVE);
            const Piece movedPiece = position.movedPiece(move);
            Int32 base = 0;
            ...
            base += CONT_HIST_BASE_CONT1_HIST_WEIGHT * contHistScore(stack, ply, 1, move, movedPiece);
            base += CONT_HIST_BASE_CONT2_HIST_WEIGHT * contHistScore(stack, ply, 2, move, movedPiece);
            base += CONT_HIST_BASE_CONT4_HIST_WEIGHT * contHistScore(stack, ply, 4, move, movedPiece);
            base += CONT_HIST_BASE_CONT6_HIST_WEIGHT * contHistScore(stack, ply, 6, move, movedPiece);
            base /= 1024;

            updateContHist(stack, ply, 1, move, movedPiece, bonus * CONT1_HIST_UPDATE_WEIGHT / 1024, base);
            updateContHist(stack, ply, 2, move, movedPiece, bonus * CONT2_HIST_UPDATE_WEIGHT / 1024, base);
            updateContHist(stack, ply, 4, move, movedPiece, bonus * CONT4_HIST_UPDATE_WEIGHT / 1024, base);
            updateContHist(stack, ply, 6, move, movedPiece, bonus * CONT6_HIST_UPDATE_WEIGHT / 1024, base);
        }

        void updateQuietHists(const Position& position, std::span<const HistoryStackEntry> stack, Move move, USize ply,
            Int32 bonus) noexcept {
            assert(move != Move::NULL_MOVE);
            updateMainPawnHists(position, move, bonus);
            updateContHist(position, stack, move, ply, bonus);
        }
        ...
        constexpr ContHistSubtable& contHistSubtable(const Position& position, Move move) noexcept {
            assert(move != Move::NULL_MOVE);
            const Piece movedPiece = position.movedPiece(move);
            return contHistTable_[movedPiece.index()][move.to().index()];
        }
        ...
        constexpr ContCorrHistSubtable& contCorrHistSubtable(const Position& position, Move move) noexcept {
            ...
            return contCorrHistTable_[movedPiece.index()][move.to().index()];
        }
        ...
        static constexpr Int32 bonus(Int32 fdepth, Int32 depthScale, Int32 offset, Int32 max) noexcept {
            return std::clamp(depthScale * fdepth / FDEPTH_SCALE - offset, 0, max);
        }
    ...
    private:
        ...
        constexpr Int32 mainHistScore(Move move, Bitboard threats, Color color) const noexcept {
            ...
            return mainHistTable_[color.index()][move.fromTo()][fromThreat][toThreat].value();
        }

        constexpr Int32 pawnHistScore(Move move, Piece movedPiece, UInt64 hash) const noexcept {
            assert(move != Move::NULL_MOVE);
            return pawnHistTable_[hash % PAWN_HIST_SIZE][movedPiece.index()][move.to().index()].value();
        }

        constexpr Int32 contHistScore(std::span<const HistoryStackEntry> stack, USize ply, USize offset, Move move,
            Piece movedPiece) const noexcept {
            if (ply >= offset && stack[ply - offset].contHistSubtable != nullptr) {
                return (*stack[ply - offset].contHistSubtable)[movedPiece.index()][move.to().index()].value();
            }
            return 0;
        }
        ...
        constexpr Int32 captHistScore(Move move, Bitboard threats, Piece movedPiece, Piece capturedPiece) const noexcept {
            ...
            const bool fromThreat = threats.get(move.from().index());
            const bool toThreat = threats.get(move.to().index());
            return captHistTable_[capturedPiece.type().index()][movedPiece.index()][move.to().index()][fromThreat][toThreat].value();
        }
        ...
        void updateMainHist(Move move, Bitboard threats, Color color, Int32 bonus) noexcept {
            ...
            const bool fromThreat = threats.get(move.from().index());
            const bool toThreat = threats.get(move.to().index());
            mainHistTable_[color.index()][move.fromTo()][fromThreat][toThreat].update(bonus * MAIN_HIST_UPDATE_WEIGHT / 1024);
        }

        void updatePawnHist(Move move, Piece movedPiece, UInt64 hash, Int32 bonus) noexcept {
            assert(move != Move::NULL_MOVE);
            pawnHistTable_[hash % PAWN_HIST_SIZE][movedPiece.index()][move.to().index()].update(bonus * PAWN_HIST_UPDATE_WEIGHT / 1024);
        }

        void updateContHist(std::span<const HistoryStackEntry> stack, USize ply, USize offset, Move move, Piece movedPiece, Int32 bonus,
            Int32 base) noexcept {
            if (ply >= offset && stack[ply - offset].contHistSubtable != nullptr) {
                (*stack[ply - offset].contHistSubtable)[movedPiece.index()][move.to().index()].update(bonus, base);
            }
        }
        ...
        void updateCaptHist(Move move, Bitboard threats, Piece movedPiece, Piece capturedPiece, Int32 bonus) noexcept {
            ...
            const bool fromThreat = threats.get(move.from().index());
            const bool toThreat = threats.get(move.to().index());
            captHistTable_[capturedPiece.type().index()][movedPiece.index()][move.to().index()][fromThreat][toThreat].update(
                bonus * CAPTURE_HIST_UPDATE_WEIGHT / 1024);
        }
    };
    ...
    public:
        ...
        void reset() noexcept {
            ...
            std::memset(&winningThreatsCorrHistTable_, 0, sizeof(winningThreatsCorrHistTable_));
        }

        Int32 correction(const Position& position, std::span<const HistoryStackEntry> stack, USize ply) const noexcept {
            ...
            correction += WINNING_THREATS_CORR_HIST_WEIGHT * winningThreatsCorrHistScore(position, color);

            const Move prevMove = (ply > 0) ? stack[ply - 1].move : Move::NULL_MOVE;
            Piece prevPiece = (ply > 0) ? stack[ply - 1].movedPiece : Piece::NONE;
            if (prevPiece == Piece::NONE) { prevPiece = Piece(PieceType::PAWN, ~color); }

            correction += CONT1_CORR_HIST_WEIGHT * contCorrHistScore(stack, ply, 1, prevMove, prevPiece);
            correction += CONT2_CORR_HIST_WEIGHT * contCorrHistScore(stack, ply, 2, prevMove, prevPiece);
            correction += CONT4_CORR_HIST_WEIGHT * contCorrHistScore(stack, ply, 4, prevMove, prevPiece);
            correction += CONT6_CORR_HIST_WEIGHT * contCorrHistScore(stack, ply, 6, prevMove, prevPiece);

            return correction / 1024;
        }

        void updateCorrHist(const Position& position, std::span<const HistoryStackEntry> stack, USize ply, Int32 fdepth, Int32 searchScore,
            Int32 staticEval) noexcept {
            const Color color = position.sideToMove();
            ...
            updateWinningThreatsCorrHist(position, color, bonus);

            const Move prevMove = (ply > 0) ? stack[ply - 1].move : Move::NULL_MOVE;
            Piece prevPiece = (ply > 0) ? stack[ply - 1].movedPiece : Piece::NONE;
            if (prevPiece == Piece::NONE) { prevPiece = Piece(PieceType::PAWN, ~color); }

            updateContCorrHist(stack, ply, 1, prevMove, prevPiece, bonus);
            updateContCorrHist(stack, ply, 2, prevMove, prevPiece, bonus);
            updateContCorrHist(stack, ply, 4, prevMove, prevPiece, bonus);
            updateContCorrHist(stack, ply, 6, prevMove, prevPiece, bonus);
        }
    ...
    private:
        ...
        WinningThreatsCorrHistTable winningThreatsCorrHistTable_;

        constexpr Int32 pawnCorrHistScore(const Position& position, Color color) const noexcept {
            return pawnCorrHistTable_[color.index()][position.pawnHash() % CORR_HIST_SIZE].value();
        }

        constexpr Int32 friendlyNonPawnCorrHistScore(const Position& position, Color color) const noexcept {
            return nonPawnCorrHistTable_[color.index()][color.index()][position.nonPawnHash(color) % CORR_HIST_SIZE].value();
        }
        ...
        constexpr Int32 minorPieceCorrHistScore(const Position& position, Color color) const noexcept {
            return minorPieceCorrHistTable_[color.index()][position.minorPieceHash() % CORR_HIST_SIZE].value();
        }

        constexpr Int32 majorPieceCorrHistScore(const Position& position, Color color) const noexcept {
            return majorPieceCorrHistTable_[color.index()][position.majorPieceHash() % CORR_HIST_SIZE].value();
        }

        constexpr Int32 winningThreatsCorrHistScore(const Position& position, Color color) const noexcept {
            const UInt64 threatHash = Utils::murmurHash3((position.winningThreats()).bits());
            return winningThreatsCorrHistTable_[color.index()][threatHash % CORR_HIST_SIZE].value();
        }

        constexpr Int32 contCorrHistScore(std::span<const HistoryStackEntry> stack, USize ply, USize offset, Move move,
            Piece movedPiece) const noexcept {
            if (ply >= offset && stack[ply - offset].contCorrHistSubtable != nullptr) {
                return (*stack[ply - offset].contCorrHistSubtable)[movedPiece.index()][move.to().index()].value();
            }
            return 0;
        }

        void updatePawnCorrHist(const Position& position, Color color, Int32 bonus) noexcept {
            pawnCorrHistTable_[color.index()][position.pawnHash() % CORR_HIST_SIZE].update(bonus);
        }

        void updateFriendlyNonPawnCorrHist(const Position& position, Color color, Int32 bonus) noexcept {
            nonPawnCorrHistTable_[color.index()][color.index()][position.nonPawnHash(color) % CORR_HIST_SIZE].update(bonus);
        }
        ...
        void updateMinorPieceCorrHist(const Position& position, Color color, Int32 bonus) noexcept {
            minorPieceCorrHistTable_[color.index()][position.minorPieceHash() % CORR_HIST_SIZE].update(bonus);
        }

        void updateMajorPieceCorrHist(const Position& position, Color color, Int32 bonus) noexcept {
            majorPieceCorrHistTable_[color.index()][position.majorPieceHash() % CORR_HIST_SIZE].update(bonus);
        }

        void updateWinningThreatsCorrHist(const Position& position, Color color, Int32 bonus) noexcept {
            const UInt64 threatHash = Utils::murmurHash3((position.winningThreats()).bits());
            winningThreatsCorrHistTable_[color.index()][threatHash % CORR_HIST_SIZE].update(bonus);
        }

        void updateContCorrHist(std::span<const HistoryStackEntry> stack, USize ply, USize offset, Move move, Piece movedPiece,
            Int32 bonus) noexcept {
            if (ply >= offset && stack[ply - offset].contCorrHistSubtable != nullptr) {
                (*stack[ply - offset].contCorrHistSubtable)[movedPiece.index()][move.to().index()].update(bonus);
            }
        }
    };
}
