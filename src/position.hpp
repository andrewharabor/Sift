...
namespace Sift {
    ...
    public:
        enum class Side : UInt8 { WHITE_KINGSIDE = 1 << 0, WHITE_QUEENSIDE = 1 << 1, BLACK_KINGSIDE = 1 << 2, BLACK_QUEENSIDE = 1 << 3 };
        ...
        static constexpr bool kingside(Side side) noexcept { return side == Side::WHITE_KINGSIDE || side == Side::BLACK_KINGSIDE; }
        ...
        static constexpr Side closestSide(Square square, Square kingSquare, Color color) noexcept {
            ...
            UInt8 shift = 0;
            if (color == Color::BLACK) { shift = 2; }
            if (square < kingSquare) { shift += 1; }
    ...
    public:
        ...
        bool set(std::string_view fen) {
            ...
            for (char c : board) {
                if (c == '/') {
                    ...
                    addPiece<true>(piece, Square(static_cast<UInt8>(index)), dummy);
                    index++;
                }
            }

            if (pieces(PieceType::KING, Color::WHITE).empty() || pieces(PieceType::KING, Color::BLACK).empty()) { return false; }
        ...
        std::string fen() const {
            ...
            for (Int32 rank = 7; rank >= 0; rank--) {
                ...
                if (emptyCount > 0) { str += std::to_string(emptyCount); }
                if (rank > 0) { str += '/'; }
            }
            ...
            if (state().castlingRights.empty()) {
                ...
                if (state().castlingRights.get(CastlingRights::WHITE_KINGSIDE)) { str += 'K'; }
                if (state().castlingRights.get(CastlingRights::WHITE_QUEENSIDE)) { str += 'Q'; }
                if (state().castlingRights.get(CastlingRights::BLACK_KINGSIDE)) { str += 'k'; }
                if (state().castlingRights.get(CastlingRights::BLACK_QUEENSIDE)) { str += 'q'; }
            }
        ...
        void makeMove(Move move, Observer& observer) noexcept {
            ...
            const Piece finalPiece = (move.type() == MoveType::PROMOTION) ? Piece(move.promotion(), sideToMove_) : movedPiece;
            const Piece capturedPiece = pieceAt(move.to());
            const PieceType pieceType = movedPiece.type();
            ...
            if (move.type() == MoveType::CASTLING) {
                ...
                kingTo = CastlingRights::kingTo(castlingSide);
                const Piece king = movedPiece;
                const Piece rook = capturedPiece;
            ...
            if (pieceType == PieceType::KING) { observer.kingMove(sideToMove_, move.from(), kingTo); }
        ...
        void unmakeMove() noexcept {
            ...
            if (move.type() == MoveType::CASTLING) {
                ...
                removePiece<false>(king, kingTo, dummy);
                removePiece<false>(rook, rookTo, dummy);
                addPiece<false>(king, move.from(), dummy);
                addPiece<false>(rook, move.to(), dummy);
                ...
                removePiece<false>(promotionPiece, move.to(), dummy);
                addPiece<false>(pawn, move.from(), dummy);
                ...
                removePiece<false>(movedPiece, move.to(), dummy);
                addPiece<false>(movedPiece, move.from(), dummy);
                ...
                if (move.type() == MoveType::EN_PASSANT) {
                    ...
                    addPiece<false>(pawn, pawnSquare, dummy);
        ...
        constexpr bool attacked(Square square, Color color) const noexcept {
            ...
            if (Attacks::pawn(square, ~color) & pieces(PieceType::PAWN, color)) { return true; }
            ...
            if (Attacks::rook(square, occupied()) & ((pieces(PieceType::ROOK, color) | pieces(PieceType::QUEEN, color)))) { return true; }
        ...
        constexpr bool check(Move move) const noexcept {
            ...
            const auto findSniper = [this](Square kingSq, Bitboard occ) {
                const Bitboard bishops =
                    Attacks::bishop(kingSq, occ) & (pieces(PieceType::BISHOP, sideToMove_) | pieces(PieceType::QUEEN, sideToMove_));
                const Bitboard rooks =
                    Attacks::rook(kingSq, occ) & (pieces(PieceType::ROOK, sideToMove_) | pieces(PieceType::QUEEN, sideToMove_));
            ...
            if (checks & toBitboard) { return true; }
        ...
        constexpr bool directCheck(Move move) const noexcept {
            ...
            if (pieceType == PieceType::KING) { return false; }

            const Bitboard checkZone = [&] {
                if (pieceType == PieceType::QUEEN) {
                    return state().checkZones[static_cast<USize>(PieceType::BISHOP)] |
                           state().checkZones[static_cast<USize>(PieceType::ROOK)];
                }
        ...
        constexpr bool legal(Move move) const noexcept {
            ...
            if (piece == Piece::NONE || piece.color() != sideToMove_) { return false; }

            if (capturedPiece != Piece::NONE &&
                ((capturedPiece.color() == sideToMove_ && (move.type() != MoveType::CASTLING || piece.type() != PieceType::KING ||
                                                              capturedPiece != Piece(PieceType::ROOK, sideToMove_))) ||
            ...
            if (piece.type() == PieceType::KING && move.type() == MoveType::NORMAL) {
                if (state().threats.get(toIndex) || !Attacks::king(from).get(toIndex)) { return false; }
                return true;
            }

            const Square kingSq = kingSquare(sideToMove_);

            if (state().checks >= 2 && piece.type() != PieceType::KING) { return false; }

            if (move.type() == MoveType::CASTLING) {
                if (state().checks != 0) { return false; }

                if (!from.backRank(sideToMove_) || !to.backRank(sideToMove_)) { return false; }

                if (piece != Piece(PieceType::KING, sideToMove_) || capturedPiece != Piece(PieceType::ROOK, sideToMove_)) { return false; }

                const CastlingRights::Side castlingSide = CastlingRights::closestSide(to, from, sideToMove_);
                if (!state().castlingRights.get(castlingSide)) { return false; }

                if (castlingPath(castlingSide) & occ) { return false; }

                const Square kingTo = CastlingRights::kingTo(castlingSide);
                if (Attacks::between(from, kingTo) & state().threats) { return false; }

                return true;
            } else if (move.type() == MoveType::EN_PASSANT) {
                if (state().enPassantSquare == Square::NONE || to != state().enPassantSquare) { return false; }

                assert(capturedPiece == Piece::NONE);

                if (piece.type() != PieceType::PAWN || !Attacks::pawn(from, sideToMove_).get(toIndex)) { return false; }

                const Square captureSq = Square(to.file(), from.rank());
                if (!state().checkMask.get(captureSq) && !state().checkMask.get(toIndex)) { return false; }
                ...
                if (possiblePin && (Attacks::rook(kingSq, occ ^ enPassantPawns) & enemyRooksQueens)) { return false; }
            ...
            if (!state().checkMask.get(toIndex)) { return false; }

            if (piece.type() == PieceType::PAWN) {
                const Rank promotionRank = Rank(Rank::EIGHTH, sideToMove_);
                if ((to.rank() == promotionRank) != (move.type() == MoveType::PROMOTION)) { return false; }
                ...
                if (from.rank() == startRank && to == from + up + up) { return pieceAt(from + up) == Piece::NONE; }

                return false;
            } else if (piece.type() == PieceType::KNIGHT) {
                if (orthogonalPinMask().get(fromIndex) || diagonalPinMask().get(fromIndex)) { return false; }
                ...
                if (diagonalPinMask().get(fromIndex)) { return (Attacks::bishop(from, occ) & diagonalPinMask()).get(toIndex); }

                if (orthogonalPinMask().get(fromIndex)) { return (Attacks::rook(from, occ) & orthogonalPinMask()).get(toIndex); }
        ...
        constexpr Piece capturedPiece(Move move) const noexcept {
            if (move.type() == MoveType::EN_PASSANT) { return Piece(PieceType::PAWN, ~sideToMove_); }

            if (move.type() == MoveType::CASTLING) { return Piece::NONE; }
        ...
        constexpr Int32 materialScore() const noexcept {
            return pieces(PieceType::PAWN).count() + (3 * pieces(PieceType::KNIGHT).count()) + (3 * pieces(PieceType::BISHOP).count()) +
                   (5 * pieces(PieceType::ROOK).count()) + (9 * pieces(PieceType::QUEEN).count());
        }

        bool repetition3Fold(USize ply) const noexcept {
            return state().repetitions > 1 || (state().repetitions == 1 && state().repetitionPly < ply);
        }
        ...
        constexpr bool insufficientMaterial() const noexcept {
            ...
            if (count == 3) {
                if (pieces(PieceType::BISHOP) || pieces(PieceType::KNIGHT)) { return true; }
            }

            if (count == 4) {
                if (pieces(PieceType::BISHOP, Color::WHITE) && pieces(PieceType::BISHOP, Color::BLACK) &&
                    Square::sameColor(pieces(PieceType::BISHOP, Color::WHITE).lsb(), pieces(PieceType::BISHOP, Color::BLACK).lsb())) {
                    return true;
                }
                ...
                if (whiteBishops.count() == 2) {
                    if (Square::sameColor(whiteBishops.lsb(), whiteBishops.msb())) { return true; }
                } else if (blackBishops.count() == 2) {
                    if (Square::sameColor(blackBishops.lsb(), blackBishops.msb())) { return true; }
                }
            }
        ...
        bool see(Move move, Int32 margin) const noexcept {
            ...
            if (move.type() == MoveType::CASTLING) { return margin <= 0; }
            ...
            while (true) {
                ...
                if (bool(pinners(color) & currOccupied)) { friendlyAttackers &= ~allPinned | allPinnedAligned; }

                if (friendlyAttackers.empty()) { break; }
    ...
    private:
        ...
        static constexpr std::array<Bitboard, 4> CASTLING_PATH_BITBOARDS = {Bitboard(Square::F1) | Bitboard(Square::G1),
            Bitboard(Square::B1) | Bitboard(Square::C1) | Bitboard(Square::D1), Bitboard(Square::F8) | Bitboard(Square::G8),
        ...
        void updateChecks() noexcept {
            ...
            if (!mask) { mask = Bitboard(0xFFFFFFFFFFFFFFFFULL); }
        ...
        void updateThreats() noexcept {
            ...
            while (queens) { threats |= Attacks::queen(queens.pop(), occ); }
        ...
        std::pair<Bitboard, Bitboard> pins() const noexcept {
            ...
            Bitboard sliders =
                Attacks::slider<PIECE_TYPE_ENUM>(kingSq, enemyOcc) & (pieces(PIECE_TYPE_ENUM) | pieces(PieceType::QUEEN)) & enemyOcc;
