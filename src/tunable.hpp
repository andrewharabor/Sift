...
TUNABLE(TTABLE_REPLACE_DEPTH_MARGIN, 2, 0, 0, 0); // FIXME: hand tune?
TUNABLE(TTABLE_REPLACE_DEPTH_PV_SCALE, 2, 0, 0, 0); // FIXME: hand tune?
...
TUNABLE(STABILITY_SCALE_MIN_DEPTH, 6, 0, 0, 0);  // FIXME: hand tune?
...
TUNABLE(AVG_SCORE_INTERPOLATION_COEFF, 8, 0, 0, 0); // FIXME: hand tune?

TUNABLE(TIME_SCALE_MIN, 7, 0, 0, 0);

TUNABLE(CORR_HISTORY_MAX, 8091, 0, 0, 0);
TUNABLE(CORR_HISTORY_UPDATE_MAX, 2009, 0, 0, 0);
TUNABLE(CORR_HISTORY_DEPTH_WEIGHT_SCALE, 2, 0, 0, 0); // FIXME: hand tune?
TUNABLE(CORR_HISTORY_DEPTH_WEIGHT_MAX, 16, 0, 0, 0); // FIXME: hand tune?

TUNABLE(PAWN_CORR_HISTORY_WEIGHT, 418, 0, 0, 0);
TUNABLE(FRIENDLY_NONPAWN_CORR_HISTORY_WEIGHT, 387, 0, 0, 0);
TUNABLE(ENEMY_NONPAWN_CORR_HISTORY_WEIGHT, 273, 0, 0, 0);
TUNABLE(THREATS_CORR_HISTORY_WEIGHT, 228, 0, 0, 0);
TUNABLE(MINOR_PIECE_CORR_HISTORY_WEIGHT, 266, 0, 0, 0);
TUNABLE(MAJOR_PIECE_CORR_HISTORY_WEIGHT, 404, 0, 0, 0);

static constexpr USize MIN_CONT_CORR_HISTORY_PLY = 2;
static constexpr USize MAX_CONT_CORR_HISTORY_PLY = 7;

using ContCorrHistoryWeightsTable = std::array<Int32, MAX_CONT_CORR_HISTORY_PLY + 1>;

static inline ContCorrHistoryWeightsTable CONT_CORR_HISTORY_WEIGHTS;

namespace ContCorrHistoryWeights {

static void init();

}

TUNABLE_CALLBACK(CONT_CORR_HISTORY_WEIGHT2, 349, 0, 0, 0, []() { ContCorrHistoryWeights::init(); });
TUNABLE_CALLBACK(CONT_CORR_HISTORY_WEIGHT3, 175, 0, 0, 0, []() { ContCorrHistoryWeights::init(); });
TUNABLE_CALLBACK(CONT_CORR_HISTORY_WEIGHT4, 230, 0, 0, 0, []() { ContCorrHistoryWeights::init(); });
TUNABLE_CALLBACK(CONT_CORR_HISTORY_WEIGHT5, 219, 0, 0, 0, []() { ContCorrHistoryWeights::init(); });
TUNABLE_CALLBACK(CONT_CORR_HISTORY_WEIGHT6, 163, 0, 0, 0, []() { ContCorrHistoryWeights::init(); });
TUNABLE_CALLBACK(CONT_CORR_HISTORY_WEIGHT7, 142, 0, 0, 0, []() { ContCorrHistoryWeights::init(); });

static inline void ContCorrHistoryWeights::init() { CONT_CORR_HISTORY_WEIGHTS = {0, 0, CONT_CORR_HISTORY_WEIGHT2, CONT_CORR_HISTORY_WEIGHT3, CONT_CORR_HISTORY_WEIGHT4, CONT_CORR_HISTORY_WEIGHT5, CONT_CORR_HISTORY_WEIGHT6, CONT_CORR_HISTORY_WEIGHT7}; }
...
TUNABLE(WINDOW_MIN_DEPTH, 6, 0, 0, 0); // FIXME: hand tune?
TUNABLE(WINDOW_MAX_DEPTH_REDUCTION, 5, 0, 0, 0); // FIXME: hand tune?
TUNABLE(WINDOW_WIDENING_COEFF, 58, 0, 0, 0);
TUNABLE(WINDOW_WIDENING_SCALE, 256, 0, 0, 0);

TUNABLE(HIGH_COMPLEXITY_MARGIN, 87, 0, 0, 0);

TUNABLE(RFP_MAX_DEPTH, 8, 0, 0, 0); // FIXME: hand tune?
...
TUNABLE(RAZORING_MAX_DEPTH, 3, 0, 0, 0); // FIXME: hand tune?
TUNABLE(RAZORING_MARGIN, 456, 0, 0, 0);
TUNABLE(RAZORING_MAX_ALPHA, 2000, 0, 0, 0);

TUNABLE(NMP_MIN_DEPTH, 2, 0, 0, 0); // FIXME: hand tune?
...
TUNABLE(NMP_NO_VERIFICATION_MAX_DEPTH, 15, 0, 0, 0); // FIXME: hand tune?
TUNABLE(NMP_MIN_PLY_DEPTH_SCALE, 3, 0, 0, 0); // FIXME: hand tune?
TUNABLE(NMP_MIN_PLY_DEPTH_DIVISOR, 4, 0, 0, 0); // FIXME: hand tune?

TUNABLE(PROBCUT_MIN_DEPTH, 5, 0, 0, 0); // FIXME: hand tune?
TUNABLE(PROBCUT_BETA_MARGIN, 190, 0, 0, 0);
TUNABLE(PROBCUT_TABLE_DEPTH_MARGIN, 3, 0, 0, 0); // FIXME: hand tune?
TUNABLE(PROBCUT_REDUCTION, 4, 0, 0, 0);

TUNABLE(IIR_MIN_DEPTH, 4, 0, 0, 0); // FIXME: hand tune?
TUNABLE(IIR_TABLE_DEPTH_MARGIN, 5, 0, 0, 0); // FIXME: hand tune?
...
TUNABLE(LMR_MIN_DEPTH, 3, 0, 0, 0); // FIXME: hand tune?
TUNABLE(LMR_MIN_MOVES_PV, 4, 0, 0, 0); // FIXME: hand tune?
TUNABLE(LMR_MIN_MOVES_NON_PV, 3, 0, 0, 0); // FIXME: hand tune?
...
TUNABLE(FP_MAX_DEPTH, 8, 0, 0, 0); // FIXME: hand tune?
...
TUNABLE(NOISY_FP_MAX_DEPTH, 5, 0, 0, 0); // FIXME: hand tune?
TUNABLE(NOISY_FP_BASE_MARGIN, 4, 0, 0, 0);
TUNABLE(NOISY_FP_DEPTH_SCALE, 113, 0, 0, 0);
TUNABLE(NOISY_FP_HIST_DIVISOR, 253, 0, 0, 0);
...
TUNABLE(HISTORY_PRUNING_MAX_DEPTH, 7, 0, 0, 0); // FIXME: hand tune?
TUNABLE(HISTORY_PRUNING_MARGIN, -1743, 0, 0, 0);
TUNABLE(HISTORY_BETA_MARGIN, 39, 0, 0, 0);

TUNABLE(SE_ROOT_DEPTH_SCALE, 2, 0, 0, 0); // FIXME: hand tune?
TUNABLE(SE_MIN_DEPTH, 5, 0, 0, 0); // FIXME: hand tune?
TUNABLE(SE_TABLE_DEPTH_MARGIN, 3, 0, 0, 0); // FIXME: hand tune?
TUNABLE(SE_BETA_SCALE, 52, 0, 0, 0);
TUNABLE(SE_BETA_SCALE_PV, 21, 0, 0, 0);
TUNABLE(SE_BETA_DEPTH_DIVISOR, 64, 0, 0, 0);
TUNABLE(SE_DEPTH_OFFSET, -1, 0, 0, 0); // FIXME: hand tune?
TUNABLE(SE_DEPTH_DIVISOR, 2, 0, 0, 0); // FIXME: hand tune?
TUNABLE(SE_DOUBLE_EXT_MARGIN, 10, 0, 0, 0);
TUNABLE(SE_TRIPLE_EXT_MARGIN, 124, 0, 0, 0);

TUNABLE(QSEARCH_MAX_MOVES, 2, 0, 0, 0); // FIXME: hand tune?
...
TUNABLE(EVAL_ADJUST_HALF_MOVE_SCALE, 200, 0, 0, 0); // FIXME: don't tune?
