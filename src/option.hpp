...
namespace Sift {
    enum class OptionType { CHECK, SPIN, STRING, BUTTON, NONE };

    struct CheckOption { bool value; };
    ...
    public:
        ...
        Option(std::string_view name, CheckOption data, Callback callback) :
            type_(OptionType::CHECK), name_(name), id_(name), data_(std::in_place_type<CheckOption>, data), callback_(std::move(callback)) {
            Utils::stringToLower(id_);
        }

        Option(std::string_view name, SpinOption data, Callback callback) :
            type_(OptionType::SPIN), name_(name), id_(name), data_(std::in_place_type<SpinOption>, data), callback_(std::move(callback)) {
            Utils::stringToLower(id_);
        }

        Option(std::string_view name, StringOption data, Callback callback) :
            type_(OptionType::STRING),
            name_(name),
            id_(name),
            data_(std::in_place_type<StringOption>, data),
            callback_(std::move(callback)) {
            Utils::stringToLower(id_);
        }

        Option(std::string_view name, Callback callback) :
            type_(OptionType::BUTTON), name_(name), id_(name), data_(std::in_place_type<std::monostate>), callback_(std::move(callback)) {
            Utils::stringToLower(id_);
        }
