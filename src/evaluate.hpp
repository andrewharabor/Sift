...
public:
    static Score evaluate(const Position &position) noexcept {
        static constexpr Score colorMultipliers[2] = {1, -1};
        ...
        const Score mopUp = floorDiv(MOP_UP_VALUE * (14 - Square::manhattanDistance(kingSquares[0], kingSquares[1])), 14);
        if (scores[1] > 0) {
            scores[1] += mopUp;
        } else if (scores[1] < 0) {
            scores[1] -= mopUp;
...
private:
    static constexpr Score PIECE_VALUES[2][6] = {
            {100, 411, 445, 582, 1250, 0},
            {115, 343, 362, 624, 1141, 0}
    };

    static constexpr Score PIECE_TABLES[2][6][64] = {
        {
    ...
    static constexpr Score PIECE_TROPISM_VALUES[2][6] = {
        {20, 82, 89, 116, 250, 0},
        {38, 114, 120, 208, 380, 0}
    };

    static constexpr Score MOP_UP_VALUE = 230;

    static constexpr Score PIECE_PHASES[6] = {0, 1, 1, 2, 4, 0};
    static constexpr Score TOTAL_PHASE = 24;

    static Score phase(const Position &position) noexcept {
    ...
    static Score interpolate(Score midgameScore, Score endgameScore, Score phase) noexcept {
        return floorDiv((midgameScore * (256 - phase)) + (endgameScore * phase), 256);
    }

    static constexpr Score floorDiv(Score a, Score b) noexcept {
        if ((a < 0) != (b < 0) && a % b != 0) {
            return a / b - 1;
        } else {
            return a / b;
