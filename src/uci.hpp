...
namespace Sift {
    ...
    public:
        UCI() :
            position_(),
            legalMoves_(),
            search_([this](const SearchInfo& info) { searchInfo(info); }, [this](Move move) { bestMove(move); },
                [this](Move move, Int32 moveNum, Int32 depth) { currMove(move, moveNum, depth); }) {
            OPTIONS.add(Option("Hash",
                SpinOption(OptionList::DEFAULT_HASH_MB, OptionList::DEFAULT_HASH_MB, OptionList::MIN_HASH_MB, OptionList::MAX_HASH_MB),
                [this](const Option&) { search_.resizeTT(); }));
            OPTIONS.add(Option("ClearHash", [this](const Option&) { search_.newGame(); }));
            OPTIONS.add(Option("Threads",
                SpinOption(OptionList::DEFAULT_THREADS, OptionList::DEFAULT_THREADS, OptionList::MIN_THREADS, OptionList::MAX_THREADS),
                [this](const Option&) { search_.setThreads(); }));
            OPTIONS.add(Option("MultiPV",
                SpinOption(OptionList::DEFAULT_MULTI_PV, OptionList::DEFAULT_MULTI_PV, OptionList::MIN_MULTI_PV, OptionList::MAX_MULTI_PV),
                [](const Option&) {}));
            OPTIONS.add(Option("Contempt",
                SpinOption(OptionList::DEFAULT_CONTEMPT, OptionList::DEFAULT_CONTEMPT, OptionList::MIN_CONTEMPT, OptionList::MAX_CONTEMPT),
                [](const Option&) {}));
            OPTIONS.add(Option("MoveOverhead",
                SpinOption(OptionList::DEFAULT_MOVE_OVERHEAD_MS, OptionList::DEFAULT_MOVE_OVERHEAD_MS, OptionList::MIN_MOVE_OVERHEAD_MS,
                    OptionList::MAX_MOVE_OVERHEAD_MS),
                [](const Option&) {}));
            OPTIONS.add(Option("ShowWDL", CheckOption(OptionList::DEFAULT_SHOW_WDL), [](const Option&) {}));

#if defined(EXTERNAL_TUNE)
            for (Tunable* tunable : TUNABLES) {
                OPTIONS.add(Option(tunable->name(), SpinOption(tunable->value(), tunable->value(), tunable->min(), tunable->max()),
                    [tunable](const Option& option) { tunable->update(static_cast<Int32>(option.spinValue())); }));
            }
#endif
    ...
    private:
        ...
        std::unique_lock<std::mutex> lockStdout() const { return std::unique_lock<std::mutex>(stdoutMutex_); }
        ...
        void uci() const {
            ...
            for (const auto& option : OPTIONS) {
                ...
                if (option.type() == OptionType::CHECK) {
                    ...
                    std::cout << "spin default " << option.spinData().defaultValue << " min " << option.spinData().min << " max "
                              << option.spinData().max << std::endl;
        ...
        void position(std::istringstream& stream) {
            ...
            while (stream >> token) {
                ...
                if (index >= legalMoves_.size()) { return; }
            ...
            while (stream >> token) {
                if (token == "wtime") {
                    ...
                    while (stream >> token) {
                        auto compare = [&token](Move move) { return std::string(move) == token; };
                        USize index = legalMoves_.findIf(compare);
                        if (index >= legalMoves_.size()) { continue; }
        ...
        void stop() {
            if (search_.running()) { search_.stop(); }
        }
        ...
        void searchInfo(const SearchInfo& info) {
            ...
            if (info.lowerBound) { std::cout << " lowerbound"; }
            if (info.upperBound) { std::cout << " upperbound"; }
            ...
            std::cout << " pv ";
            for (const Move move : info.pv) { std::cout << std::string(move) << " "; }
        ...
        void setOption(std::istringstream& stream) {
            ...
            stream >> token;
            if (option.type() != OptionType::BUTTON && token != "value") { return; }
        ...
        void perftTests() {
            ...
            for (USize i = 0; i < Perft::TEST_COUNT; i++) {
                ...
                if (nodes == testCase.expectedNodes) { testsPassed++; }
        ...
        void moves() {
            ...
            genLegalMoves();
            for (const Move move : legalMoves_) { std::cout << std::string(move) << " "; }
            std::cout << std::endl;
        }
        ...
        void hash() const {
            std::unique_lock<std::mutex> lock = lockStdout();
            std::cout << std::showbase << std::uppercase << std::hex << position_.hash() << std::dec << std::nouppercase << std::noshowbase
                      << std::endl;
        }
