...
namespace Sift {
    enum class TTBound : UInt8 { NONE, EXACT, LOWER, UPPER };
    ...
    struct TTEntry {
        ...
        constexpr TTEntry() noexcept :
            score(Score::NONE), staticEval(Score::NONE), move(Move::NULL_MOVE), fdepth(0), pv(false), bound(TTBound::NONE) {}
    };
    ...
    public:
        ...
        void write(UInt64 hash, Int32 ply, Int32 score, Int32 staticEval, Move move, Int32 fdepth, bool pv, TTBound bound) noexcept {
            ...
            for (USize i = 0; i < CLUSTER_SIZE; i++) {
                ...
                const Int32 entryQuality =
                    quality(static_cast<Int32>(cluster.entries[i].age()), static_cast<Int32>(cluster.entries[i].fdepth));
            ...
            if (bound == TTBound::EXACT || replace.hash16 != hash16 || replace.age() != age_ ||
                fdepth + TT_REPLACE_DEPTH_OFFSET + (TT_REPLACE_PV_SCALE * pv) > static_cast<Int32>(replace.fdepth)) {
                if (move != Move::NULL_MOVE || replace.hash16 != hash16) { replace.move = move; }
        ...
        USize hashfull() const noexcept {
            ...
            for (USize i = 0; i < sampleSize; i++) {
                for (USize j = 0; j < CLUSTER_SIZE; j++) {
                    ...
                    if (entry.filled && entry.age() == age_) { count++; }
                }
            }
    ...
    private:
        ...
        struct RawEntry {
            ...
            void setBoundPVAge(TTBound bound, bool pv, UInt8 age) {
                boundPVAge = static_cast<UInt8>(bound) | (static_cast<UInt8>(pv << 2) | static_cast<UInt8>(age << 3));
            }
        };
        ...
        Int32 retrieve(Int16 score, Int32 ply) const noexcept {
            if (Score::mate(score)) { return (score < 0) ? (score + ply) : (score - ply); }
            return score;
        }

        Int16 store(Int32 score, Int32 ply) const noexcept {
            if (Score::mate(score)) { return (score < 0) ? static_cast<Int16>(score - ply) : static_cast<Int16>(score + ply); }
