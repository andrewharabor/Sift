...
private:
    ...
    inline void accumulateThreatChanges(std::span<Int16, L1_SIZE> acc, const FeatureTransformer &ft, std::span<const UInt16> adds, std::span<const UInt16> subs) noexcept {
        ...
        for (USize i = 0; i < CHUNKS; i += TILE) {
            ...
            for (const UInt16 idx : subs) {
                ...
                for (USize j = 0; j < TILE; j++) {
                    tmp[j] = SIMD::sub<Int16>(tmp[j], SIMD::widenLoadInt8(&sub[(i + j) * CHUNK_SIZE]));
                }
            }
            ...
            for (const UInt16 idx : adds) {
                ...
                for (USize j = 0; j < TILE; j++) {
                    tmp[j] = SIMD::add<Int16>(tmp[j], SIMD::widenLoadInt8(&add[(i + j) * CHUNK_SIZE]));
                }
            }

            for (USize j = 0; j < TILE; j++) {
                SIMD::store<Int16>(&acc[(i + j) * CHUNK_SIZE], tmp[j]);
            }
        }
    }
    ...
    inline void writePPChanges(Color color, Square kingSquare, Bitboard whiteBefore, Bitboard blackBefore, Bitboard whiteAfter, Bitboard blackAfter, std::span<UInt16> adds, std::span<UInt16> subs, USize &addOffset, USize &subOffset) noexcept {
...
#if defined(USE_PEXT)
        const auto bandMask = [&](Square square) -> UInt16 { return static_cast<UInt16>(_pext_u64(PPFeature::MASKS[square.index()] & same, same)); };
#else
        ...
        const auto bandMask = [&](Square square) -> UInt16 { return static_cast<UInt16>(slowPEXT(PPFeature::MASKS[square.index()] & same, same)); };
#endif
