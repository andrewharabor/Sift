...
namespace Sift {
    ...
    public:
        constexpr TimeManager() noexcept :
            startTime_(), softBound_(), hardBound_(), checkCount_(0), prevBestMove_(), stability_(1), averageScore_(0) {}
        ...
        void limits(const SearchLimits& limits, Color color, USize moveCount) noexcept {
            if (limits.clock.enabled) {
                ...
                const Float64 baseTimeScale =
                    static_cast<Float64>((limits.clock.movesToGo > 0) ? limits.clock.movesToGo : TIME_BASE_TIME_SCALE);
        ...
        bool stopSoft(const SearchLimits& limits, Int32 depth, Move bestMove, Int32 score, UInt64 bestMoveNodes, UInt64 nodes) noexcept {
            if (limits.clock.enabled &&
                elapsed() > std::chrono::duration_cast<MS>(softBound_ * softBoundScale(depth, bestMove, score, bestMoveNodes, nodes))) {
                return true;
            }
        ...
        bool stopHard(const SearchLimits& limits, UInt64 nodes, USize numThreads) noexcept {
            if (nodes * static_cast<UInt64>(numThreads) >= limits.nodes) { return true; }

            if (++checkCount_ == CHECK_INTERVAL) {
                checkCount_ = 0;
                if (elapsed() > limits.time) { return true; }
                if (limits.clock.enabled && elapsed() > hardBound_) { return true; }
            }
            return false;
        }
    ...
    private:
        ...
        constexpr Float64 softBoundScale(Int32 depth, Move bestMove, Int32 score, UInt64 bestMoveNodes, UInt64 nodes) noexcept {
            ...
            if (Score::mate(score)) { return Utils::floatDiv100(TIME_SCALE_MATE_SCORE); }

            if (Score::decisive(score)) { return Utils::floatDiv100(TIME_SCALE_DECISIVE_SCORE); }

            Float64 scale = 1.0;

            const Float64 nodeFraction = static_cast<Float64>(bestMoveNodes) / static_cast<Float64>(nodes + 1);
            scale *= std::max(Utils::floatDiv100(TIME_SCALE_NODE_MIN),
                Utils::floatDiv100(TIME_SCALE_NODE_BASE) - (nodeFraction * Utils::floatDiv100(TIME_SCALE_NODE_SCALE)));

            if (depth >= TIME_SCALE_STABILITY_MIN_DEPTH) {
                scale *= std::min(Utils::floatDiv100(TIME_SCALE_STABILITY_MAX),
                    Utils::floatDiv100(TIME_SCALE_STABILITY_BASE) +
                        (Utils::floatDiv100(TIME_SCALE_STABILITY_SCALE) *
                            std::pow(static_cast<Float64>(stability_) + Utils::floatDiv100(TIME_SCALE_STABILITY_OFFSET),
                                Utils::floatDiv100(TIME_SCALE_STABILITY_POWER))));
            }

            const Float64 scoreChange = static_cast<Float64>(score - averageScore_) / Utils::floatDiv100(TIME_SCALE_SCORE_CHANGE_DIVISOR);
            const Float64 scoreScaleSignCoeff =
                (scoreChange > 0) ? Utils::floatDiv100(TIME_SCALE_POS_SCORE_SCALE) : Utils::floatDiv100(TIME_SCALE_NEG_SCORE_SCALE);
            const Float64 invScale = (scoreChange * Utils::floatDiv100(TIME_SCALE_SCORE_SCALE)) /
                                     (std::abs(scoreChange) + Utils::floatDiv100(TIME_SCALE_SCORE_OFFSET)) * scoreScaleSignCoeff;
