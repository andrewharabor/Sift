...
public:
    ...
    void limits(const SearchLimits &limits, Color color, USize moveCount) noexcept {
        if (limits.clock.enabled) {
            ...
            const auto baseTime = (time / baseTimeScale) + (increment * float64(TIME_INCREMENT_SCALE));

            softBound_ = std::chrono::duration_cast<MS>(baseTime * float64(TIME_SOFT_SCALE));
            hardBound_ = std::chrono::duration_cast<MS>(time * float64(TIME_HARD_SCALE));
...
private:
    ...
    constexpr Float64 softBoundScale(Int32 depth, Move bestMove, Int32 score, UInt64 bestMoveNodes, UInt64 nodes) noexcept {
        ...
        if (Score::mate(score)) {
            return float64(TIME_MATE_SCORE_SCALE);
        }

        if (score >= Score::KNOWN_WIN) {
            return float64(TIME_WIN_SCORE_SCALE);
        }

        if (score <= Score::KNOWN_LOSS) {
            return float64(TIME_LOSS_SCORE_SCALE);
        }

        Float64 scale = 1.0;

        const Float64 nodeFraction = static_cast<Float64>(bestMoveNodes) / static_cast<Float64>(nodes + 1);
        scale *= std::max(float64(TIME_NODE_SCALE_MIN), float64(TIME_NODE_SCALE_BASE) - (nodeFraction * float64(TIME_NODE_SCALE_COEFF)));

        if (depth >= STABILITY_SCALE_MIN_DEPTH) {
            scale *= std::min(float64(TIME_STABILITY_SCALE_MAX), float64(TIME_STABILITY_SCALE_MIN) + (float64(TIME_STABILITY_SCALE_COEFF) * std::pow(static_cast<Float64>(stability_) + float64(TIME_STABILITY_SCALE_OFFSET), float64(TIME_STABILITY_SCALE_POWER))));
        }

        const Float64 scoreChange = static_cast<Float64>(score - averageScore_) / float64(TIME_SCORE_SCALE_CHANGE_COEFF);
        const Float64 scoreScaleSignCoeff = (scoreChange > 0) ? float64(TIME_SCORE_SCALE_POS_SCALE) : float64(TIME_SCORE_SCALE_NEG_SCALE);
        const Float64 invScale = (scoreChange * float64(TIME_SCORE_SCALE_COEFF)) / (std::abs(scoreChange) + float64(TIME_SCORE_SCALE_OFFSET)) * scoreScaleSignCoeff;
        scale *= std::clamp(1.0 - invScale, float64(TIME_SCORE_SCALE_MIN), float64(TIME_SCORE_SCALE_MAX));
        averageScore_ = (averageScore_ * (AVG_SCORE_INTERPOLATION_COEFF - 1) + score) / AVG_SCORE_INTERPOLATION_COEFF;

        scale = std::max(scale, float64(TIME_SCALE_MIN));
        return scale;
    }

    static constexpr Float64 float64(Int32 value, Float64 divisor = 100.0) noexcept { return static_cast<Float64>(value) / divisor; }
};
