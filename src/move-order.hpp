...
namespace Sift {
    ...
    inline std::strong_ordering operator<=>(MoveOrderStage stage1, MoveOrderStage stage2) {
        return static_cast<UInt8>(stage1) <=> static_cast<UInt8>(stage2);
    }

    class MoveOrder {
    public:
        static inline MoveOrder search(MoveList& moves, const Position& position, const History& history,
            std::span<const HistoryStackEntry> histStack, Move ttMove, USize ply) noexcept {
            return MoveOrder(MoveOrderStage::TT, moves, position, history, histStack, ttMove, ply);
        }

        static inline MoveOrder qsearch(MoveList& moves, const Position& position, const History& history,
            std::span<const HistoryStackEntry> histStack, Move ttMove, USize ply, bool forceEvasions) {
            const MoveOrderStage stage =
                (forceEvasions || position.inCheck()) ? (MoveOrderStage::QSEARCH_EVASIONS_TT) : (MoveOrderStage::QSEARCH_TT);
            return MoveOrder(stage, moves, position, history, histStack, ttMove, ply);
        }

        static inline MoveOrder probcut(MoveList& moves, const Position& position, const History& history,
            std::span<const HistoryStackEntry> histStack, Move ttMove, USize ply) noexcept {
            return MoveOrder(MoveOrderStage::PROBCUT_TT, moves, position, history, histStack, ttMove, ply);
        }
        ...
        Move next() noexcept {
            if (stage_ == MoveOrderStage::TT) {
                ...
                if (ttMove_ != Move::NULL_MOVE && position_.legal(ttMove_)) { return ttMove_; }
            }
            ...
            if (stage_ == MoveOrderStage::QUIET) {
                if (!skipQuiets_) {
                    if (const Move move = selectNext<true>(); move != Move::NULL_MOVE) { return move; }
                }
            ...
            if (stage_ == MoveOrderStage::BAD_NOISY) {
                if (const Move move = selectNext<false>(); move != Move::NULL_MOVE) { return move; }
    ...
    private:
        ...
        explicit MoveOrder(MoveOrderStage initialStage, MoveList& moves, const Position& position, const History& history,
            std::span<const HistoryStackEntry> histStack, Move ttMove, USize ply) noexcept :
            stage_(initialStage),
            moves_(moves),
            moveScores_(),
        ...
        constexpr void scoreQuiets() noexcept {
            for (USize i = idx_; i < end_; i++) {
                ...
                score +=
                    MOVE_ORDER_DIRECT_CHECK_BONUS * (position_.directCheck(move) && position_.see(move, MOVE_ORDER_DIRECT_CHECK_MARGIN));
            }
        }
