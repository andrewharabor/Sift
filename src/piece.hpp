...
namespace Sift {
    class PieceType {
    public:
        enum class PieceTypeEnum : UInt8 { PAWN, KNIGHT, BISHOP, ROOK, QUEEN, KING, NONE };
    ...
    public:
        ...
        constexpr Piece(PieceType pieceType, Color color) noexcept : piece_(PieceEnum::NONE) {
            if (pieceType != PieceType::NONE && color != Color::NONE) { piece_ = static_cast<PieceEnum>(color * 6 + pieceType); }
        }
        ...
        constexpr PieceType type() const noexcept {
            return (piece_ == PieceEnum::NONE) ? PieceType::NONE : PieceType(static_cast<UInt8>(piece_) % 6);
        }
